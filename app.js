...
function createDummyImage(emoji) {
  const bgColors = {
    '🥩':'1a0900', '🍔':'1a0900', '🍟':'1a0900', '🔥':'1a0900',
    '🍖':'1a0900', '🍗':'1a0900', '👨‍👩‍👧‍👦':'1a0900', '🏆':'1a0900',
    '🍱':'1a0900', '⚡':'1a0900', '🍋':'0a1200', '🍊':'0a0800',
    '🥤':'000a14', '💧':'000a14'
  };
  const bg = bgColors[emoji] || '1a0900';
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="200" height="200">
    <rect width="200" height="200" fill="%23${bg}"/>
...
function saveCart() {
  try { localStorage.setItem('budare_cart', JSON.stringify(cart)); }
  catch(e) { console.warn('No se pudo guardar el carrito'); }
}
  ...
  try {
    ...
    if (saved) {
      ...
      Object.keys(parsed).forEach(id => {
        if (!MENU.find(p => p.id === id)) delete parsed[id];
      });
      cart = parsed;
    } else {
      cart = {};
...
function badgeHTML(badges) {
  const map = {
    hot: '<span class="badge b-hot">🔥 Más vendido</span>',
    star:'<span class="badge b-star">⭐ Recomendado</span>',
    new: '<span class="badge b-new">Nuevo</span>'
  };
...
/* CART PANEL */
function openCart() {
  if (cartCount() === 0) { showToast('🛒 Tu carrito está vacío'); return; }
  renderCartBody();
  ...
  if (entries.length === 0) {
    document.getElementById('cartBody').innerHTML = `
      <div class="cart-empty">
        ...
        <p class="cart-empty-text">Tu carrito está vacío.<br>Agrega productos del menú.</p>
      </div>`;
    return;
  }
  document.getElementById('cartBody').innerHTML = entries.map(([id, qty]) => {
    ...
    return `<div class="cart-item">
      ...
      <div class="ci-info">
        ...
        <p class="ci-price">${fmtPrice(p.price * qty)}</p>
      </div>
      <div class="ci-ctrl">
  ...
  document.getElementById('checkoutSummary').innerHTML = `
    ...
    ${Object.entries(cart).map(([id, qty]) => {
      ...
      return `<div class="os-item">
        <span class="os-item-name">${p.name}</span>
        <span class="os-item-qty">×${qty}</span>
        <span class="os-item-price">${fmtPrice(p.price * qty)}</span>
      </div>`;
    }).join('')}
    <div class="os-total">
      <span class="os-total-label">Total</span>
      <span class="os-total-val">${fmtPrice(cartTotal())}</span>
    </div>`;
}
...
function sendOrder() {
  ...
  const msg = `🔥 *PEDIDO — BUDARE GRILL*\n\n` +
    `🧾 *Productos:*\n${lines}\n\n` +
    `💰 *Total: ${fmtPrice(cartTotal())}*\n\n` +
    `👤 Nombre: ${name}\n` +
    `📍 Dirección: ${address || 'Para recoger en el local'}\n` +
